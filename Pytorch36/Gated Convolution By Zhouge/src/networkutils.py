...
# network : GatedConv2dWithActivation
...
class SNGatedConv2dWithActivation(torch.nn.Module):
    ...
    """
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=False, batch_norm=True, activation=torch.nn.LeakyReLU(0.2, inplace=True)):
...
class SNGatedDeConv2dWithActivation(torch.nn.Module):
    ...
    Output:\phi(f(I))*\sigmoid(g(I))
    """
    def __init__(self, scale_factor, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=False, batch_norm=True, activation=torch.nn.LeakyReLU(0.2, inplace=True)):
...
class SNConvWithActivation(torch.nn.Module):
    ...
    """
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=False, activation=torch.nn.LeakyReLU(0.2, inplace=True)):
...
if __name__ == '__main__':
    ...
    # sngated = nn.Sequential(nn.ConvTranspose2d(3,64,2,1,0))
    # summary(sngated,input_size=[(3,100,100)],batch_size=1,device='cpu')
    sngated2 = SNGatedDeConv2dWithActivation(2,3,64,2,1,1)
    summary(sngated2,input_size=[(3,100,100)],batch_size=1,device='cpu')
