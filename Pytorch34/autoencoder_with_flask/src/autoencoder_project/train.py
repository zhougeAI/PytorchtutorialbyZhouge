...
class Autoencoder_training():
    def __init__(self, config):
        ...
        if config.pretrained:
            saved_state_dict = torch.load(config.state_dict_path)
            ...
            self.epoch = 0
        self.train_dataset = MyDataset(dataset_path=config.train_dataset_path)
        self.eval_dataset = MyDataset(dataset_path=config.eval_dataset_path)
        self.train_dataloader = DataLoader(self.train_dataset, batch_size=config.train_batch_size, shuffle=True)
        self.eval_dataloader = DataLoader(self.eval_dataset, batch_size=config.eval_batch_size, shuffle=False)
        ...
        # 1. 是优化器
        # 2. 损失函数
        self.optimizer = torch.optim.Adam(self.autoencoder.parameters(), lr=config.learning_rate)
        ...
        for current_train_epochs in range(self.epoch, self.max_train_epochs):
            ...
            if current_train_epochs % self.save_model_interval == 0:
                ...
                torch.save({'model': self.autoencoder.state_dict(),
                            ...
                            'loss': loss.item()},
                           os.path.join(config.checkpoints_path, "epoch_{}.pth".format(current_train_epochs)))
    ...
    def sample(self, current_train_epochs):
        ...
        image_board.save( os.path.join(config.train_results_path, "{}.png".format(current_train_epochs)))
        print("saving sample {}.png at {}".format(current_train_epochs, config.train_results_path))
